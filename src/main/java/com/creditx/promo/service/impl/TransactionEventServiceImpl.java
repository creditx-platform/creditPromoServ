...
public class TransactionEventServiceImpl implements TransactionEventService {
    ...
    var promos = promotionRepository.findByStartDateLessThanEqualAndExpiryDateGreaterThanEqual(
        event.getCreatedAt(), event.getCreatedAt())
        .stream()
        .filter(p -> p.getStatus() != PromotionStatus.INACTIVE) // allow ACTIVE or EXPIRED (retroactive)
        .toList();
    ...
    try {
        ...
        restTemplate.postForEntity(creditMainServiceUrl + "/api/transactions/cashback", entity, String.class);
        application = PromotionApplication.builder()
            .applicationId(java.util.UUID.randomUUID().toString())
            .promoId(promo.getPromoId())
            .transactionId(event.getTransactionId())
            .issuerId(event.getIssuerAccountId())
            .merchantId(event.getMerchantAccountId())
        ...
        log.error("Cashback creation failed for promo {} txn {}", promo.getPromoId(), event.getTransactionId(), e);
        application = PromotionApplication.builder()
            .applicationId(java.util.UUID.randomUUID().toString())
            .promoId(promo.getPromoId())
            .transactionId(event.getTransactionId())
