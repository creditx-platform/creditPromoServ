...
class PromoEvaluatorServiceImplTest {
  ...
  @BeforeEach
  void setup() {
    testEvent = TransactionPostedEvent.builder().transactionId(10L).issuerAccountId(1L)
        .merchantAccountId(200L).amount(new BigDecimal("100")).currency("USD")
        .type(TransactionType.INBOUND).createdAt(Instant.parse("2025-06-01T00:00:00Z")).build();

    testPromotion = Promotion.builder().promoId("P1").name("Test Promo")
        ...
        .rewardFormula("{\"cashbackPercent\":10,\"maxCashback\":25}").status(PromotionStatus.ACTIVE)
        .build();
  }
  ...
  void shouldFilterOutInactivePromotions() {
    // given
    Promotion inactivePromotion = Promotion.builder().promoId("INACTIVE").name("Inactive Promo")
        .startDate(Instant.parse("2025-01-01T00:00:00Z"))
        .expiryDate(Instant.parse("2025-12-31T00:00:00Z")).eligibilityRules("{\"minAmount\":50}")
        .rewardFormula("{\"cashbackPercent\":5,\"maxCashback\":10}")
        .status(PromotionStatus.INACTIVE).build();
  ...
  void shouldFilterOutPromotionsWithAmountTooLow() {
    // given
    TransactionPostedEvent lowAmountEvent = TransactionPostedEvent.builder().transactionId(11L)
        .issuerAccountId(1L).merchantAccountId(200L).amount(new BigDecimal("10")).currency("USD")
        .type(TransactionType.INBOUND).createdAt(Instant.parse("2025-06-01T00:00:00Z")).build();
  ...
  void shouldFilterOutPromotionsWithWrongMerchant() {
    // given
    TransactionPostedEvent wrongMerchantEvent = TransactionPostedEvent.builder().transactionId(12L)
        .issuerAccountId(1L).merchantAccountId(999L) // Different merchant
        .amount(new BigDecimal("100")).currency("USD").type(TransactionType.INBOUND)
        .createdAt(Instant.parse("2025-06-01T00:00:00Z")).build();
  ...
  void shouldFilterOutExpiredPromotions() {
    // given
    TransactionPostedEvent futureEvent = TransactionPostedEvent.builder().transactionId(13L)
        .issuerAccountId(1L).merchantAccountId(200L).amount(new BigDecimal("100")).currency("USD")
  ...
  void shouldAllowPromotionWithoutMerchantRestriction() {
    // given
    Promotion noMerchantPromo = Promotion.builder().promoId("P2").name("No Merchant Restriction")
        .startDate(Instant.parse("2025-01-01T00:00:00Z"))
        .expiryDate(Instant.parse("2025-12-31T00:00:00Z")).eligibilityRules("{\"minAmount\":50}")
        .rewardFormula("{\"cashbackPercent\":5,\"maxCashback\":10}").status(PromotionStatus.ACTIVE)
        .build();
  ...
  void shouldHandleInvalidJson() {
    // given
    Promotion invalidJsonPromo = Promotion.builder().promoId("INVALID").name("Invalid JSON Promo")
        .startDate(Instant.parse("2025-01-01T00:00:00Z"))
        .expiryDate(Instant.parse("2025-12-31T00:00:00Z")).eligibilityRules("not-valid-json")
        .rewardFormula("{\"cashbackPercent\":5,\"maxCashback\":10}").status(PromotionStatus.ACTIVE)
        .build();
